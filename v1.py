...
def main():
    ...
    for i in range(num_tables):
        ...
        num_rows = st.number_input(f"Number of rows to generate for {table_name}", min_value=1, max_value=10000, value=10)
    ...
    for table_name, columns in tables.items():
        ...
        for col in columns:
            if col["type"] == "VARCHAR":
                ...
                data[col["name"]] = data_frames[ref_table][ref_column].values
    ...
    selected_table = st.selectbox("Select a table to visualise", list(data_frames.keys()))
    if selected_table:
        df = data_frames[selected_table]
...
"""
                ...
                try:
                    ...
                    sql_query = chat_completion.choices[0].message.content.split('
sql')[-1].split('
')[0].strip()
                    st.text_area("Generated SQL Query", value=sql_query, height=100)
                    summary_prompt = f"""
...
"""
                    ...
                    try:
                        ...
                        # Option to ask a clarification question after the data have been served
...
sql
        ...
        Query Result: {result.head().to_string(index=False)}
